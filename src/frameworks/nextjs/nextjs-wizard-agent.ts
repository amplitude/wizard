  ...
  prompts: {
    ...
    getAdditionalContextLines: (context) => {
      ...
      return [
        ...
        `Preferred BROWSER SDK: @amplitude/unified — single package wrapping @amplitude/analytics-browser, Session Replay, and Guides & Surveys (engagement). Initialize from a 'use client' boundary (root layout for App Router, _app.tsx for Pages Router) so it loads in the browser bundle, never the server bundle. The init call MUST follow the wizard commandments: full autocapture block + remoteConfig.fetchRemoteConfig in the analytics block, sessionReplay: { sampleRate: 1 }, and engagement: {}. All three ship with the unified package and the wizard auto-enables them so users get Amplitude's full out-of-the-box coverage. Every option line MUST have an inline // comment explaining what it does — that's the user's opt-out surface (comment out the line for any feature they don't want).`,
