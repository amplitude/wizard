  ...
  prompts: {
    ...
    getAdditionalContextLines: (context) => {
      ...
      return [
        ...
        `Preferred BROWSER SDK: @amplitude/unified — single package wrapping @amplitude/analytics-browser. Accepts the same config options as the browser SDK. Initialize from a 'use client' boundary (root layout for App Router, _app.tsx for Pages Router) so it loads in the browser bundle, never the server bundle. Use the autocapture defaults from the wizard commandments (fetchRemoteConfig + the full autocapture block) to match Amplitude's CDN snippet. Session Replay and Guides & Surveys ARE available through @amplitude/unified but the wizard treats them as explicit opt-ins — only add the sessionReplay plugin or engagement block when sessionReplayOptIn / engagementOptIn is set on the session.`,
