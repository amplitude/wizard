...
/**
 * Matches POSIX stderr/stdout-redirection tokens like `2>&1`, `1>&2`, `>&2`.
 * Used to normalize commands before allowlist/operator checks so the safe
 * redirection idiom doesn't trip the dangerous-operator deny.
 *
 * Stable shared constant: previously this regex was inlined three times
 * (`isSafeBackgroundedInstall`, the `run_in_background` allow path, and the
 * main normalizer in `wizardCanUseTool`). One source of truth keeps the
 * three sites guaranteed-equivalent if the pattern ever needs adjusting.
 */
const STDERR_REDIRECTION_PATTERN = /\s*\d*>&\d+\s*/g;

/**
 * Extract the `command` string from a Bash tool input record. Centralizes
 * the `typeof === 'string' ? ... : ''` defensive coercion that appears at
 * every entry point in `createPreToolUseHook` / `wizardCanUseTool`. Returns
 * `''` (never `undefined`) so the call sites can call `.match`, `.trim`,
 * etc. without a null-check.
 */
function readBashCommand(toolInput: Record<string, unknown>): string {
...
export function isSafeBackgroundedInstall(command: string): boolean {
  ...
  const stripped = command.replace(STDERR_REDIRECTION_PATTERN, ' ').trim();
...
export function wizardCanUseTool(
  ...
  if (
    ...
    !/[|&]/.test(command.replace(STDERR_REDIRECTION_PATTERN, ' '))
  ) {
  ...
  const normalized = command.replace(STDERR_REDIRECTION_PATTERN, ' ').trim();
...
/**
 * Build a PreToolUse `deny` hook return envelope. The Claude Agent SDK
 * expects exactly this shape (see `HookCallback` in `agent-hooks.ts`):
 *
 *   {
 *     hookSpecificOutput: {
 ...
 *   }
 *
 * The `permissionDecisionReason` field is opaque to the SDK — most call
 * sites in this file pass a JSON-encoded `toWizardToolDenyMessage` envelope
 * so the agent gets a structured deny payload, but the load_skill loop
 * guard passes a plain string for a more conversational message. Both are
 * permitted; the SDK forwards the string verbatim to the model.
 *
 * Centralizing the literal here removes the five identical inline copies
 * in `createPreToolUseHook` (load_skill loop, destructive-bash deny,
 * scanner-exception fail-closed, long-sleep deny, allowlist delegate
 * deny) — if the SDK ever extends the shape we only update one place.
 */
function buildPreToolUseDenyEnvelope(
  ...
  return (input: Record<string, unknown>) => {
    ...
    if (toolName === 'mcp__wizard-tools__load_skill') {
      ...
      if (skillId) {
        ...
        if (count > MAX_LOAD_SKILL_PER_ID) {
          ...
          return Promise.resolve(buildPreToolUseDenyEnvelope(reason));
        }
      }
    }
      ...
      try {
        ...
        if (scan.matched && scan.rule) {
          ...
          return Promise.resolve(
            trackBashDeny(
              buildPreToolUseDenyEnvelope(structured),
              command,
              scan.rule.message,
            ),
          );
    ...
    if (toolName === 'Bash') {
      ...
      if (decision.behavior === 'deny') {
        ...
        return Promise.resolve(
          trackBashDeny(
            buildPreToolUseDenyEnvelope(decision.message),
            command,
            humanReason,
          ),
        );
