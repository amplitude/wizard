...
// Single source of truth: the labels the agent must use in TodoWrite are
// defined in `canonical-tasks.ts` and consumed both by the store (which
// renders them) and by these tests (which lock the system prompt to them).
