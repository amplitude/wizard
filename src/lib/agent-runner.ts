...
STEP 6: Add event tracking to this project using the instrumentation skills.
   ...
   - Implement the tracking calls for **all priority-3 (critical) AND all priority-2 (useful) events** identified by the skill, plus any events the user explicitly requests via confirm_event_plan feedback. The instrument-events skill defaults to priority-3 only — for the wizard run you MUST also include priority-2 events. Do not silently drop events to keep the plan small. If the user says "also add X, Y, Z" or proposes 8 events themselves via the feedback loop, instrument every one of them — even if the priority is 1 or unlisted. The only events you should ever exclude from the proposed plan are ones already covered by Autocapture or ones that would duplicate an event the project already instruments. Err on the side of MORE events, not fewer — the user can always delete events later, but undiscovered surfaces are far more expensive to find and instrument after the fact.
