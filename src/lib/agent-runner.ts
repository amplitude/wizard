...
async function runAgentWizardBody(
  ...
  const agentResult = await runAgentDispatch(
    ...
    spinner,
    {
      ...
      onCircuitBreakerTripped: ({ consecutiveDenies, lastCommand }) => {
        ...
        // session object but never bumps `$version`, so the screen stayed
        // on Run until a terminal resize forced an Ink redraw.
        getUI().setOutroData({
  ...
  // `$version`, so the screen stayed on Run until a terminal resize
  // forced an Ink redraw. `getUI().outro()` below sees outroData is set
  // and skips its default; it still flips runPhase → Completed which
  // emits a second change event for the router to advance.
  getUI().setOutroData({
