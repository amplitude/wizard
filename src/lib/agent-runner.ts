...
async function runAgentWizardBody(
  ...
  // PR #726: wrap the parallel cold-start call in a single phase boundary
  // so orchestrators see a `package_manager_detection` cold_start_breakdown
  // entry even though it now races against agent init rather than running
  // serially. Timing here covers BOTH legs of the parallel block; the
  // separate `framework_detection` boundary below covers `buildPreflightContext`.
  ...
  // PR #726: emit the `framework_detection` cold_start_breakdown around
  // buildPreflightContext so orchestrators get per-phase timing.
