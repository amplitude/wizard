...
async function runAgentWizardBody(
  ...
  if (
    ...
    //
    // Real-world example we just hit: ✓ Welcome ✓ Auth ✓ Setup ✓ Verify
    // ● Done — every step green-checked, but the wizard surfaced
    // "Setup cancelled" with `Detail: Amplitude MCP not connected —
    ...
    if (dashboardComplete || eventsInstrumented) {
      ...
      getUI().pushStatus(
        `Note: ${what} couldn't reach the Amplitude MCP server — your SDK + events are instrumented. ${
          dashboardComplete
            ? ''
      ...
      await wizardAbort({
        message: isMcp
          ...
          : `Couldn't load setup instructions for ${config.metadata.name}${
              detail ? `\n\nDetail: ${detail}` : ''
            }\n\nThis may be a temporary service issue or a version mismatch.\n\nTry again in a moment, or set up ${
              config.metadata.name
            } manually:\n${config.metadata.docsUrl}`,
