...
describe('inferAppType', () => {
  ...
  it('returns Full-stack web for Next.js with src/ directory layout (src/app/api)', () => {
    const srcFixture = fs.mkdtempSync(
      path.join(os.tmpdir(), 'wizard-classifier-src-test-'),
    );
    fs.mkdirSync(path.join(srcFixture, 'src', 'app', 'api'), {
      recursive: true,
    });
    try {
      expect(
        inferAppType(pkg({ next: '^14.0.0', react: '^18.0.0' }), srcFixture),
      ).toEqual({ value: 'Full-stack web' });
    } finally {
      fs.rmSync(srcFixture, { recursive: true, force: true });
    }
  });
