...
CRITICAL — confirm_event_plan format:
  ...
  `After all event and identity instrumentation is complete, write a file named \`.amplitude-events.json\` at the project root listing every event you instrumented (exact event_type, a short description, and the file path where it's tracked). Do NOT create charts or dashboards yourself — the wizard runs a dedicated post-agent step that reads this file and creates the dashboard with bounded timeouts and progress reporting. Your job ends at instrumentation + writing this file.`,
