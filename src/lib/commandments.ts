...
CRITICAL — confirm_event_plan format:
  ...
  `After all event and identity instrumentation is complete, write a file named \`.amplitude-events.json\` at the project root. Shape: \`{ "events": [ { "name": "<exact event name>", "description": "<short description>", "file": "<path where instrumented>" } ] }\`. Use the key \`name\` (matching the event_type you passed to track()) — not \`event\`, \`event_type\`, or \`eventName\`. Do NOT create charts or dashboards yourself — the wizard runs a dedicated post-agent step that reads this file and creates the dashboard with bounded timeouts and progress reporting. Your job ends at instrumentation + writing this file.`,
