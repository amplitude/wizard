...
Do NOT add a fifth — internal steps (env var writes, Content Security Policy edits, build verification, setup report, doc fetches) roll into the appropriate parent (CSP and env vars into "Install Amplitude"; setup report and build verification into "Wire up event tracking"). Engineering phases from the integration skill (1.0-begin / 1.1-edit / 1.2-revise / 1.3-conclude) are internal — they do not appear here. The denominator MUST stay 4 — the wizard renders "X / 4 tasks complete" regardless of what your list says. Chart + dashboard creation is NOT part of this run; it lives in a separate deferred command (\`amplitude-wizard dashboard\`) that runs once event ingestion has caught up — do not call \`record_dashboard\`, \`create_chart\`, \`create_dashboard\`, or any Amplitude MCP chart/dashboard tool here.`,
  ...
  `Plan size & pagination — when your candidate list has >50 events, call \`propose_event_plan\` first to get suggested batch boundaries (chunks of ~25 events; configurable via \`AMPLITUDE_WIZARD_EVENT_PLAN_CHUNK_SIZE\`). The wizard returns \`{ paginated, chunkSize, suggestedBatches: [{batchIndex, totalBatches, start, end, eventNames}] }\`. For each suggested batch, call \`confirm_event_plan\` with that batch's events plus the corresponding \`batchIndex\` and \`totalBatches\` fields, then instrument that batch BEFORE moving to the next. Each batch may also include \`callsites: [{filePath, anchor?}]\` per event so the rich plan survives a context compaction. For ≤50-event projects, skip \`propose_event_plan\` and call \`confirm_event_plan\` once with the full list (no batch fields needed) — this preserves today's single-pass behavior.`,
