...
CRITICAL — do NOT manually write \`.amplitude/events.json\` (or legacy \`.amplitude-events.json\`). \`confirm_event_plan\` persists the plan to \`.amplitude/events.json\` in the canonical \`[{name, description}]\` shape the wizard UI expects. Writing it yourself with a different shape (event_name, eventName, file_path, etc.) drifts the manifest from real track() calls.
