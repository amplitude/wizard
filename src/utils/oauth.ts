 ...
 * caller's responsibility — once the caller has resolved a real user
 * identity (via `fetchAmplitudeUser`), it should call `storeToken(user, tokens)`
 * with the returned tokens. This avoids writing partial `{id:'pending'}`
 * entries to disk.
 */
