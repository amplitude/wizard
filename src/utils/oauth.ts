...
import { detectRegionFromToken } from './urls.js';
...
/**
 * The result of an auth attempt. Every outcome guarantees that tokens have
 * been persisted to `~/.ampli.json` (so credential resolution downstream
 * can find them), and partial/failure cases are reflected explicitly in the
 * discriminator — callers match on `status` exhaustively rather than having
 * to remember persistence sequencing at each call site.
 *
 * - `complete`: userInfo fetched and real user record persisted. This is
 *   the happy path. Use `userInfo` (full profile + orgs) for UI and
 *   `user`/`tokens` if you only need the stored shapes.
 * - `pending-recovery`: tokens persisted under a `{id:'pending'}` sentinel
 *   because the userInfo fetch failed (provisioning race, network hiccup,
 *   etc.). Next run can recover via cached-session lookup; the caller can
 *   also retry the userInfo fetch on the same tokens if it wants to try
 *   to complete in the current run.
 */
...
/**
 * Extends {@link AuthOutcome} with a `skipped` state for code paths where
 * an auth attempt may be a no-op (feature flag off, missing inputs, etc.).
 * Only returned by `performSignupOrAuth`; `performAmplitudeAuth` always
 * attempts the OAuth flow.
 */
export type SignupOutcome = AuthOutcome | { status: 'skipped' };

/**
 * Resolve the data zone for userInfo fetching. EU users authenticate through
 * the US OAuth endpoint but their account data lives in the EU. `detectRegionFromToken`
 * probes both data planes and returns whichever responds. Falls back to the
 * auth zone on any detection error.
 */
async function resolveDataZone(
  accessToken: string,
  authZone: AmplitudeZone,
): Promise<AmplitudeZone> {
  try {
    return (await detectRegionFromToken(accessToken)) as AmplitudeZone;
  } catch {
    return authZone;
  }
}
...
/**
 * Given a freshly-resolved token set, fetch the Amplitude user profile and
 * persist a complete record. On fetch failure, persist a `{id:'pending'}`
 * sentinel so the tokens aren't lost — next run recovers via cached-session
 * lookup without forcing the user back through browser OAuth.
 */
async function completeOrRecover(
  tokens: StoredOAuthToken,
  authZone: AmplitudeZone,
): Promise<AuthOutcome> {
  const dataZone = await resolveDataZone(tokens.accessToken, authZone);
  try {
    const userInfo = await fetchAmplitudeUser(tokens.idToken, dataZone);
    ...
    logToFile('[oauth] completeOrRecover → complete', {
      zone: dataZone,
      userId: userInfo.id,
    });
    return { status: 'complete', user, userInfo, tokens, zone: dataZone };
  } catch (err) {
    logToFile('[oauth] completeOrRecover → pending (userInfo fetch failed)', {
      zone: dataZone,
    ...
    return { status: 'pending-recovery', tokens, zone: dataZone };
  }
}

/**
 * Performs the Amplitude OAuth2/PKCE flow and returns a persisted auth state.
 *
 * 1. If `forceFresh` is false, tries the existing `~/.ampli.json` session first.
 * 2. Otherwise (or on no cached session), opens the browser for fresh OAuth.
 * 3. Detects the user's data zone and fetches their profile.
 * 4. Persists the result to disk — either as a complete record or, if the
 *    userInfo fetch failed, under a `{id:'pending'}` sentinel for next-run recovery.
 *
 * Always returns an {@link AuthOutcome} — callers don't need to call
 * `storeToken` themselves, and forgetting persistence is structurally impossible.
 */
export async function performAmplitudeAuth(options: {
  ...
  if (!options.forceFresh) {
    ...
    if (existing) {
      ...
      return completeOrRecover(existing, zone);
    }
  }
  ...
  try {
    ...
    return completeOrRecover(tokens, zone);
  } catch (e) {
