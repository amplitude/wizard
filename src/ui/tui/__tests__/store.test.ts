...
describe('WizardStore', () => {
  ...
  describe('session setters', () => {
    ...
    it('setRunPhase(Running) pre-populates the canonical 5 tasks', () => {
      // The user-visible progress list must show the journey ahead from
      // frame 1 — staring at an empty list while the agent cold-starts is
      // a known abandonment moment. We pre-populate at the Running
      // transition so the denominator is `5` immediately.
      const store = createStore();
    ...
    it('setRunPhase(Running) does not clobber an existing task list', () => {
      // Re-entering Running (or constructing in an already-Running state
      // for tests) must not wipe progress already accumulated by syncTodos.
      const store = createStore();
      store.setRunPhase(RunPhase.Running);
      store.syncTodos([{ content: 'Install Amplitude', status: 'completed' }]);
      const completedBefore = store.tasks[1].status;
      store.setRunPhase(RunPhase.Idle);
      store.setRunPhase(RunPhase.Running);
      expect(store.tasks[1].status).toBe(completedBefore);
    });
  ...
  describe('syncTodos', () => {
    // The user-visible progress list is locked to the five canonical steps
    // defined in `canonical-tasks.ts`. Agent TodoWrite output is bucketed
    // into them — exact-label match first, keyword fallback after. Drift
    // in agent wording, retries, or stray sixth todos must not reorder,
    // regress, or duplicate the displayed list.

    it('renders exactly the five canonical steps regardless of incoming list length', () => {
      const store = createStore();
      store.syncTodos([
        { content: 'Install SDK', status: 'in_progress' },
        { content: 'Configure', status: 'pending' },
      ]);
    ...
    it('buckets canonical-label todos to the matching step', () => {
      const store = createStore();
      store.syncTodos([
        { content: 'Detect your project setup', status: 'completed' },
    ...
    it('buckets drifted labels to the right canonical step via keywords', () => {
      // The agent occasionally renames a step ("Install Amplitude SDK",
      // "Inspect project structure"). Keyword bucketing keeps these on
      // the right canonical row instead of dropping them or creating a
      // duplicate.
      const store = createStore();
      store.syncTodos([
        { content: 'Inspect project structure', status: 'completed' },
        { content: 'Install Amplitude SDK', status: 'in_progress' },
      ]);
    ...
    it('drops todos that match no canonical step (denominator stays 5)', () => {
      const store = createStore();
      store.syncTodos([
        { content: 'Some random unrelated task', status: 'completed' },
      ]);

      expect(store.tasks).toHaveLength(5);
      // Nothing was bucketed, so all five remain pending.
      expect(store.tasks.every((t) => t.status === TaskStatus.Pending)).toBe(
        true,
      );
    });
    ...
    it('forces monotonic progress — completed steps cannot regress', () => {
      // Regression scenario: an HTTP 400 / 429 retry causes the agent to
      // re-emit its TodoWrite list with stale state, demoting tasks that
      // were already completed. The user sees their progress visibly
      // un-check. Once a step has been completed it stays completed.
      const store = createStore();
      store.syncTodos([
        { content: 'Detect your project setup', status: 'completed' },
        { content: 'Install Amplitude', status: 'in_progress' },
      ...
      store.syncTodos([
        // Retry-induced regression — agent demotes step 1 back to in_progress.
        { content: 'Detect your project setup', status: 'in_progress' },
        { content: 'Install Amplitude', status: 'in_progress' },
      ]);
    ...
    it('preserves completed state across renamed labels', () => {
      // Variant of the regression: instead of re-emitting the same label
      // with worse status, the agent *renames* the step on retry. The
      // canonical-bucket monotonic guard catches this where the prior
      // exact-label-keyed guard did not.
      const store = createStore();
      store.syncTodos([{ content: 'Install Amplitude', status: 'completed' }]);

      // Agent retry — same step, different wording.
      store.syncTodos([
        { content: 'Install Amplitude SDK', status: 'in_progress' },
      ]);

      expect(store.tasks[1].label).toBe('Install Amplitude');
      expect(store.tasks[1].status).toBe(TaskStatus.Completed);
    });

    it('enforces single-in-progress + ordering when the agent skips ahead', () => {
      // The agent sometimes marks a later step in_progress while an
      // earlier one is still pending. The user-visible list must show
      // the earlier step completed (the agent has clearly moved on) so
      // tasks don't appear out of order.
      const store = createStore();
      store.syncTodos([
        // Agent skips Install Amplitude and goes straight to wiring up tracking.
        { content: 'Wire up event tracking', status: 'in_progress' },
      ]);

      expect(store.tasks[0].status).toBe(TaskStatus.Completed); // detect
      expect(store.tasks[1].status).toBe(TaskStatus.Completed); // install
      expect(store.tasks[2].status).toBe(TaskStatus.Completed); // plan
      expect(store.tasks[3].status).toBe(TaskStatus.InProgress); // wire
      expect(store.tasks[4].status).toBe(TaskStatus.Pending); // dashboard
    });

    it('collapses simultaneous in_progress to the latest step (later wins)', () => {
      // Two steps in_progress at once is a contract violation — the canonical
      // journey is sequential. Pick the latest as active, mark earlier ones
      // completed.
      const store = createStore();
      store.syncTodos([
        { content: 'Install Amplitude', status: 'in_progress' },
        { content: 'Plan and approve events to track', status: 'in_progress' },
      ]);

      expect(store.tasks[1].status).toBe(TaskStatus.Completed);
      expect(store.tasks[2].status).toBe(TaskStatus.InProgress);
    });

    it('preserves the latest activeForm per canonical step', () => {
      const store = createStore();
      store.syncTodos([
        {
  ...
  describe('edge cases', () => {
    ...
    it('syncTodos with empty array seeds the canonical 5 as pending', () => {
      // The user-visible list is locked to the canonical 5. An empty
      // TodoWrite from the agent means "no progress yet" — the rows
      // still need to render so the user sees the journey ahead.
      const store = createStore();
      store.syncTodos([]);
    ...
    it('syncTodos with unknown status defaults to Pending for the bucketed step', () => {
      const store = createStore();
      store.syncTodos([{ content: 'Install Amplitude', status: '' }]);
      // Step 2 ("Install Amplitude") was the bucket; with an empty status
      // it stays pending. (Other steps remain pending too.)
      expect(store.tasks[1].status).toBe(TaskStatus.Pending);
    });
