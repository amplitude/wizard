...
| `~/.amplitude/wizard/runs/<sha256(installDir)>/checkpoint.json` | Crash-recovery snapshot | `src/lib/session-checkpoint.ts` |
| `~/.amplitude/wizard/plans/<planId>.json` | `wizard plan` output | `src/lib/agent-plans.ts` |
| `~/.amplitude/wizard/state/<attemptId>.json` | Agent compaction-recovery state | `src/lib/agent-state.ts` |
| `~/.amplitude/wizard/update-check.json` | npm registry latest-version cache | `src/utils/update-notifier.ts` |
