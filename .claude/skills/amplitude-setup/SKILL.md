...
description:
  Set up Amplitude analytics in this codebase using the Amplitude Wizard agent
  contract. Use whenever a user asks to install Amplitude, instrument analytics
  events, configure Amplitude SDK, connect this app to Amplitude, or wire up
  tracking with @amplitude/wizard.
metadata:
  author: Amplitude
  version: '1.0.0'
---
...
the user-relevant ones, and stop for explicit user confirmation before any file
or Amplitude project is modified.
...
2. **Never run `apply` in the background.** Run it in the foreground so its
   NDJSON streams through your tool output in real time. Backgrounding it forces
   you to poll a tail file with `sleep && tail`, which adds minutes of latency
   between each progress update.
3. **NEVER spawn a second `apply` while one is already running.** If a previous
   `apply` invocation is still streaming events on stdout, do NOT start another.
   Each apply spawns an inner Claude SDK agent that writes files; two of them
   stomp each other's edits and produce contradictory `setup_complete` payloads.
   Wait for the current run's `run_completed` event before any retry.
...
5. **Always pass `--install-dir <abs-path>` and confirm it's a real project.**
   ...
   parent (`~/dev/`, `~/code/`, etc.), or any directory without a clear project
   marker (`package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`, `Gemfile`,
   `pom.xml`, `build.gradle`), STOP and ask the user which project to set up. Do
   not guess.
...
7. **Always render `type: diagnostic` events** as user-visible status. Transient
   retries should look like `⚠ retrying (2/6)...`, not silent gaps.
...
9. **Wizard-managed artifacts are owned by the wizard.** Do NOT use `Write` /
   ...
   shape; the dashboard watcher mirrors the dashboard JSON. Direct writes are
   denied by the inner-agent permission hook with a clear error message — if you
   see that deny, switch to the MCP tool.

## Fast path — `wizard mcp serve` (PREFERRED)

If your orchestrator can speak MCP (Claude Code, Cursor, Codex), prefer the
wizard's MCP server over shelling out to `npx`. Each `npx` call pays 5-10s of
cold start; the MCP server runs once and answers every tool call in-process.
Configure it once:

```jsonc
// .claude/mcp.json or your MCP client's config
{
  "mcpServers": {
    "amplitude-wizard": {
      "command": "npx",
...
| Tool                                                | Purpose                                                                       |
| --------------------------------------------------- | ----------------------------------------------------------------------------- |
| `detect_framework({ installDir })`                  | Auto-detect the framework. Read-only.                                         |
| `get_project_status({ installDir })`                | Full project state (framework, SDK, API key, auth). Read-only.                |
| `get_auth_status()`                                 | Login state + token expiry.                                                   |
| `plan_setup({ installDir })`                        | Build a setup plan; returns planId + proposed events. **No writes.**          |
| `apply_plan({ planId, eventDecision, appId, ... })` | Execute a plan. Streams progress to stderr; returns `setup_complete` payload. |
| `verify_setup({ installDir })`                      | Cheap health check after apply.                                               |
| `reset_project({ installDir })`                     | Wipe `.amplitude/` artifacts.                                                 |

Recommended flow:

1. `get_auth_status()` — show user who they're signed in as.
2. `plan_setup({ installDir })` — get planId + proposed events.
3. **Show events to user, get explicit approve / revise / skip.**
4. `apply_plan({ planId, eventDecision, appId? })` — pass the user's decision.
   Returns `{ exitCode, setupComplete, amplitude, eventCount }`.
5. On `exitCode === 0`, pin project context to `setupComplete.amplitude.appId`.

Stderr from `apply_plan` carries the live NDJSON stream — render relevant events
as user-visible status (see translation table below).

If MCP isn't available, fall back to the CLI flow below.

## Fast path (CLI) — `wizard go`

For one-shot scripted setups where the user has already approved the plan, skip
the explicit `plan` step and use the combined command:

```
npx @amplitude/wizard go --install-dir <abs-path> --json
```

`go` runs `plan` and `apply` in a single process: the agent sees a
`setup_context (go)` lifecycle event, then a `go_plan` envelope with the
proposed events, then the standard `apply` event stream. Pass `--app-id <id>` if
you want to skip the env-selection prompt;
`--event-decision skipped|revised --revise-feedback "<text>"` to override the
default approve.

For the **user-driven** flow where you must surface the event plan first, use
`plan` then `apply` separately (steps below).

## Step 1 — preflight (cheap, no writes)

Run in parallel; they're fast and read-only:
...
> Auth: `<email>` (Org: `<orgName>`, Region: `<region>`) Framework:
> `<frameworkName>` detected at `<install-dir>`

If `whoami.loggedIn === false`: stop and ask the user to run
`npx @amplitude/wizard login` in their terminal (browser OAuth — agents cannot
drive it). After they confirm "logged in", re-run `whoami --json` to verify.
...
2. `type: plan, data.event: plan` — the `planId`, framework, SDK package, and
   proposed events.

Render the proposed events as a short bulleted list with one line each. Ask the
user:

> "I'll instrument these N events. Approve, revise, or skip? Reply 'approve',
> 'revise: <feedback>', or 'skip'."
...
Do NOT background this. Stream the NDJSON live and surface events to the user as
they arrive. Translate each event type into one short status line:
...
- `amplitude.appId` — the ONLY project handle to use for follow-up Amplitude MCP
  queries. Disregard any earlier project context.
...
The wizard also persists this scope to `ampli.json` in the install dir (`AppId`,
`OrgId`, `ProjectId`, `EnvName`, `DashboardUrl`, `DashboardId`) so future agent
sessions can recover it without re-running setup.
...
answer, then re-invoke `apply` with the appropriate `resumeFlags` from the event
payload (or compose them yourself):
...
verbatim and let the user pick. Each choice carries `resumeFlags` for its own
re-invocation — use those rather than composing your own.
...
- Do not silently approve the event plan. The user's analytics taxonomy is their
  decision, not yours.
...
- Do not log raw NDJSON lines to the user. Translate each event into a one-line
  status using the table above.
- Do not run the wizard from the user's home directory or a multi-project parent
  dir without explicit `--install-dir`. Confirm the project first.
- Do not `Write` or `Edit` `.amplitude/events.json`,
  `.amplitude/dashboard.json`, `.amplitude-events.json`, or
  `.amplitude-dashboard.json`. They are owned by the wizard's MCP tools.

## Starting fresh — `wizard reset`

If a previous wizard run left artifacts that no longer match the user's intent
(stale event plan, wrong dashboard, instrumented for the wrong Amplitude app),
run:
...
`amplitude-setup-report.md` from the project, and strips auth-scoped fields
(`OrgId`, `AppId`, `AppName`, `EnvName`, `DashboardUrl`, `DashboardId`) from
`ampli.json`. It does NOT log the user out and does NOT remove any `track()`
calls already wired into source code — the user has to revert those manually if
they want a true blank slate (`git checkout` is the safe path; `wizard reset`
...
- The proposed event plan no longer fits the codebase (UI changed, new flows
  added).

When NOT to suggest `reset`:

- The wizard is mid-run. Wait for `run_completed`.
- The user just wants to add more events. Re-running `plan` + `apply` is enough;
  the wizard supplements the existing plan rather than replacing it.
...
catalog and `references/example-session.jsonl` for a worked happy-path session.
