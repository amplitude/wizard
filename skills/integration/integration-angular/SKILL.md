...
- Use inject() instead of constructor injection. Amplitude service should be injected via inject() in components/services that need it.
- Create a dedicated AmplitudeService as a singleton root service that wraps the Amplitude SDK.
- Always use standalone components over NgModules.
- Configure Amplitude credentials in src/environments/environment.ts files, as Angular reads environment variables from these configuration files
