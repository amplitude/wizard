...
This skill helps you add Amplitude analytics to Flask applications.
...
- Initialize Amplitude globally in create_app() using the API key from environment variables (NOT per-request)
- Blueprint registration happens AFTER Amplitude initialization in create_app()
- Add amplitude.track() calls at meaningful action points within route handlers
