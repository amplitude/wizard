# Wizard — Final Migration Plan (in-tree, AI SDK target)

**Status:** binding execution plan. Effective 2026-05-06.
**Supersedes:** `NEW_MIGRATION_PLAN.md` (PR #541) for execution sequencing. **Retains** the diagnoses, library guidance, and backwards-compat contract from `MIGRATION_PLAN.md`. **Retains** the three-tier skills delivery contract from `SKILLS_AND_CONTEXT_DESIGN.md` verbatim.
**Authoring inputs:** the two plan docs above; the 12-PR Kelson stack on `amplitude/wizard` (#541→#553); the eval-scaffolding draft #537; the signup work #534/#535/#539/#220; the classic-mode removal #558; ground-truth verification across `wizard`, `wizard-rewrite`, `wizard-v2`, `context-hub`.
...
## 1. Why this plan, and what changed since `NEW_MIGRATION_PLAN.md`

`NEW_MIGRATION_PLAN.md` got the strategic direction right (in-repo evolution, AI-SDK target, phased extraction). It is silent on three things that matter for execution:
...
|---|---|---|---|
| 1 | #541 | doc | Plants `NEW_MIGRATION_PLAN.md` + ports pure `sanitizeWizardRequestInit` tests. |
...
The contract is `SKILLS_AND_CONTEXT_DESIGN.md`. Don't restate it — implement it.
...
**Remaining:**
1. **Default the flag on** once Tier-1 cache hit rate ≥ 70% across 50 representative agent runs. Then drop `preStageSkills` and `cleanupIntegrationSkills` per `SKILLS_AND_CONTEXT_DESIGN.md` §7 PR 1.
2. **Phase commandments split** per `SKILLS_AND_CONTEXT_DESIGN.md` §3. Move `src/lib/commandments.ts` to `src/lib/commandments/{always-on, phase, framework}/*.ts` and place cache breakpoints 1-4 per §4.
...
The combination of caching + tiering + structured outputs is where the real cost/latency wins live. The `SKILLS_AND_CONTEXT_DESIGN.md` §5 estimate (2-4 minutes wall-clock saved per run, ~60-70% input-cost reduction on a typical run) holds when these are applied together; it does not hold for caching alone.
...
The `SKILLS_AND_CONTEXT_DESIGN.md` proposal is the contract for tier delivery, the prompt cache layout, and commandments triage. This plan adopts it verbatim. Two execution adjustments based on verified state:

- The disabled `load_skill_menu` / `install_skill` block in `wizard-tools.ts` is being re-enabled by the #544/#545/#546 stack in the cleaner shape proposed (single `load_skill` returning body inline, plus `load_skill_reference`). **Do not reintroduce the staging-on-disk path** the original disable rationale identified.
- The `browser-sdk-2.md` deduplication in context-hub is the single highest-value byte cut. Per §10 decision 1, scope is the **full cross-repo dedup** (paired wizard + context-hub PR). Lands in Phase C, before Phase F's packaging cut. The `SKILLS_AND_CONTEXT_DESIGN.md` §8 open question is closed by this decision.
...
- Auth: `authToken` via `@ai-sdk/anthropic` 3.x (per `MIGRATION_PLAN.md` decision §8.2).
...
- Beta-header allowlist (replace the regex pass-through at `router.ts:443-451` per `MIGRATION_PLAN.md` §6).
...
| Cache miss on every distinct orchestrator-injected `--context-file` | C/D | Place orchestrator content **after** breakpoint 4 in the dynamic suffix per `SKILLS_AND_CONTEXT_DESIGN.md` §8 risk note. |
...
- `MIGRATION_PLAN.md` — diagnoses, cherry-pick prescription, backwards-compat contract, monorepo coordination guidance.
- `NEW_MIGRATION_PLAN.md` (PR #541) — in-tree strategic direction this plan extends.
- `SKILLS_AND_CONTEXT_DESIGN.md` — token math, three-tier skills contract, prompt cache layout, commandments triage.
