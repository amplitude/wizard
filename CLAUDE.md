...
- **Event-name naming.** Every event name passed to `wizardCapture()` uses **Title Case**; the full Amplitude event name is `wizard cli: <Title Case Name>` (the `wizard cli: ` prefix is added by `wizardCapture()`). Capitalize major words, keep short connectors lowercase (`of`, `to`, `for`), and keep acronyms uppercase (`MCP`, `API`, `SDK`, `CLI`). Examples: `'Session Started'`, `'Auth Complete'`, `'MCP Install Complete'`, `'Agent SDK Error Retry'`, `'Create Project Fallback Link Opened'`.
